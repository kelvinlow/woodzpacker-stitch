...
export default {
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    const articleMatch = path.match(/^\/v1\/article\/([a-zA-Z0-9-]+)$/);
    if (articleMatch) {
...
async function handleArticleDetail(articleId: string, env: Env): Promise<Response> {
  ...
  try {
    ...
    if (!response.ok) {
      const errorData = await response.text();
      return new Response(
        JSON.stringify({ error: "Failed to fetch article content", details: errorData }),
        { status: response.status, headers: { "Content-Type": "application/json" } }
      );
    }
    ...
    const simplifiedBlocks = data.results.map((block) => {
      ...
      if (["paragraph", "heading_1", "heading_2", "heading_3", "bulleted_list_item", "numbered_list_item"].includes(type)) {
        simplified.text = content.rich_text?.map((t: any) => t.plain_text).join("") || "";
      } else if (type === "image") {
        simplified.url = content.external?.url || content.file?.url || "";
        simplified.caption = content.caption?.map((t: any) => t.plain_text).join("") || "";
      }
